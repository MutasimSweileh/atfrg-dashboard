...
<template>
  <b-row>
    <b-col cols="12" lg="12">
      <b-card no-header>
        <template slot="header"> New Genre </template>
         <template>
                  <b-col md="12">
                    <b-card>
                      <div slot="header">New Genre</div>
                      <b-form id="user-form" @submit="Addgenre">
                        <b-form-group>
                          <b-input-group>
...
export default {
  ...
  methods: {
    ...
    if(this.Validation() == true){
        ...
        this.ChangesDone = "Genre Hass Been Added Successfuly.";
        this.check = false;
        }).catch((error) => {
        this.RestErrors();
        this.NameValidation = 'This Genre already Exsist !';
        console.log(error);

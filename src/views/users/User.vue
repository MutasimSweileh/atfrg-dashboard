<template>
  <b-row>
    <b-col cols="12" lg="12">
      <b-card no-header>
        <template slot="header"> UserID : {{ $route.params.id }}   <b-button @click="dangerModal = true" type="reset" style="float:right" size="sm" variant="danger"><i class="fa fa-ban"></i> Delete</b-button></template>
        <ApolloQuery
          :query="
            gql => gql`
            ...
            <div v-else-if="data && data.users.length > 0">
              ...
              <template>
                <div v-for="user in data.users" :key="user.id">
                  <b-col md="12">
                    <b-card>
                      <div slot="header">Username : {{ user.username }}</div>
                      <b-form id="user-form" @submit="SaveChanges">
                        <b-form-group>
                          <b-input-group>
...
import gql from "graphql-tag";
 const Edit_User = gql`
  mutation UserEdit($id:ID,$pass:String!,$genreTypes:[GenreType!]!,$role:UserRole!,$activated:Boolean){
